...
export const upsertProductRecord = async (product: Stripe.Product) => {
  ...
  try {
    ...
    throw new Error(`cannot add or update product ${error}`);
  }
...
export const upsertPriceRecord = async (price: Stripe.Price) => {
  ...
  try {
    ...
    throw new Error(`cannot add or update price ${error}`);
  }
  console.log("Price added/updated:", price.id);
};
...
export const createOrRetrieveCustomer = async ({
  ...
  try {
    const res = await db.query.customers.findFirst({
      where: (c, { eq }) => eq(c.id, uuid),
    });
    if (!res) throw new Error();
    return res.stripeCustomerId;
  } catch (error) {
    ...
    try {
      ...
      throw new Error("cannot find or create customer");
    }
  }
};
...
export const copyBillingDetailsToCustomer = async (uuid: string, payment_method: Stripe.PaymentMethod) => {
    ...
    //@ts-ignore
    await stripe.customers.update(customers, {name, phone, address})
    try {
        ...
        throw new Error(`Cannot copy customer billing details`)
    }
}
...
export const manageSubscriptionStatusChange = async (subscriptionId: string, customerId: string, createAction = false) => {
    ...
    const subscriptionData: Subscription = {
      ...
      console.log(`Added/Updated subscription [${subscription.id}] for user [${uuid}]`)
